...
class ProcessingCoreBlackBoxMem(nWeightBanks: Int, sratchPadMemDepth: Int, activationMemDepth: Int,
  ...
  val weightBanks         = Array.fill(nWeightBanks){ Module(new DualPortedMemBlackBox(dataWidth, sratchPadMemDepth))}
  val activationMemBank   = Array.fill(nWeightBanks){ Module(new DualPortedMemBlackBox(dataWidth, activationMemDepth))}
  ...
  for(pe <- 0 until nWeightBanks){
    ...
    activationMemBank(pe).io.clock := clock
...
object ProcessingCoreBlackBoxGenerator extends App{
  new (ChiselStage).emitVerilog(new ProcessingCoreBlackBoxMem(32, 128, 16,
    10, 7, false), Array("--target-dir","generatedVerilog"))
}
...
class DQNAccelertorBlackBoxTop(val p: DQNAcceleratorParams) extends Module{
  val io = IO(new Bundle{
    val instruction     = Input(UInt(32.W))
  ...
  val DQNCore = Module(new ProcessingCoreBlackBoxMem(p.nWeightBanks, p.scratchPadMemDepth,
...
object DQNAcceleratorBlackBoxTopGenerator extends App{
  new (ChiselStage).emitVerilog(new DQNAccelertorBlackBoxTop(DQNAcceleratorParams.apply()),
